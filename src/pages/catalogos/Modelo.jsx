...
export default function Modelo() {
  ...
  const cargarRegistros = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/rapicredito/modelos/todos`, {
  ...
  const acciones = {
    ...
    eliminar: async (fila) => {
      if (confirm(`¿Deseas eliminar el modelo con ID ${fila.id}?`)) {
        try {
          await axios.delete(
            `${API_BASE_URL}/rapicredito/modelos/eliminar/${fila.id}`,
            {
              headers: { Authorization: `Bearer ${auth.token}` },
            }
          );
  ...
  const guardarRegistro = async () => {
    try {
      ...
      if (modoEdicion && registroActual) {
        await axios.put(
          `${API_BASE_URL}/rapicredito/modelos/actualizar/${registroActual.id}`,
          payload,
          {
            headers: { Authorization: `Bearer ${auth.token}` },
          }
        );
        setMensajeExito("Modelo actualizado correctamente");
      } else {
        await axios.post(
          `${API_BASE_URL}/rapicredito/modelos/crear`,
          payload,
          {
            headers: { Authorization: `Bearer ${auth.token}` },
          }
        );
        setMensajeExito("Modelo creado correctamente");
      }
      ...
      <main className="p-6">
        <div className="mb-6">
          ...
          <div className="mt-2 flex justify-between items-center">
            <h1 className="text-2xl font-bold text-[#002F6C]">Catálogo: Modelos</h1>
        ...
        <ModalBase
          ...
          titulo={modoEdicion ? "Editar modelo" : "Crear modelo"}
