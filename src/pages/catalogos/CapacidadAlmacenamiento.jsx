...
export default function CapacidadAlmacenamiento() {
  const { auth } = useAuth();
  ...
  const cargarRegistros = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/rapicredito/capacidad-almacenamiento`, {
  ...
  const acciones = {
    ...
    eliminar: async (fila) => {
      if (confirm(`¿Deseas eliminar la capacidad con ID ${fila.id}?`)) {
        try {
          await axios.delete(
            `${API_BASE_URL}/rapicredito/capacidad-almacenamiento/eliminar/${fila.id}`,
            {
              headers: { Authorization: `Bearer ${auth.token}` },
            }
          );
          setMensajeExito("Capacidad eliminada correctamente");
          cargarRegistros();
  ...
  const guardarRegistro = async () => {
    try {
      ...
      if (modoEdicion && registroActual) {
        await axios.put(
          `${API_BASE_URL}/rapicredito/capacidad-almacenamiento/actualizar/${registroActual.id}`,
          payload,
          {
            headers: { Authorization: `Bearer ${auth.token}` },
          }
        );
        setMensajeExito("Capacidad actualizada correctamente");
      } else {
        await axios.post(
          `${API_BASE_URL}/rapicredito/capacidad-almacenamiento/crear`,
          payload,
          {
            headers: { Authorization: `Bearer ${auth.token}` },
          }
        );
        setMensajeExito("Capacidad creada correctamente");
      }
    ...
    <>
      ...
      <main className="p-6">
        <div className="mb-6">
          ...
          <div className="mt-2 flex justify-between items-center">
            <h1 className="text-2xl font-bold text-[#002F6C]">
              Catálogo: Capacidad de Almacenamiento
            </h1>
        ...
        <ModalBase
          ...
          titulo={modoEdicion ? "Editar capacidad" : "Crear capacidad"}
