...
export default function Color() {
  ...
  const cargarRegistros = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/rapicredito/colores/todos`, {
  ...
  const acciones = {
    ...
    eliminar: async (fila) => {
      if (confirm(`¿Deseas eliminar el color con ID ${fila.id}?`)) {
        try {
          await axios.delete(
            `${API_BASE_URL}/rapicredito/colores/eliminar/${fila.id}`,
            {
              headers: { Authorization: `Bearer ${auth.token}` },
            }
          );
          setMensajeExito("Color eliminado correctamente");
          cargarRegistros();
  ...
  const guardarRegistro = async () => {
    try {
      ...
      if (modoEdicion && registroActual) {
        await axios.put(
          `${API_BASE_URL}/rapicredito/colores/actualizar/${registroActual.id}`,
          payload,
          {
            headers: { Authorization: `Bearer ${auth.token}` },
          }
        );
        setMensajeExito("Color actualizado correctamente");
      } else {
        await axios.post(
          `${API_BASE_URL}/rapicredito/colores/crear`,
          payload,
          {
            headers: { Authorization: `Bearer ${auth.token}` },
          }
        );
        setMensajeExito("Color creado correctamente");
      }
    ...
    <>
      ...
      <main className="p-6">
        ...
        <ModalBase
          ...
          titulo={modoEdicion ? "Editar color" : "Crear color"}
